...
if (isGroup) {
		...
		if (isGroupAdmins) return reply(`oi ${pushname2}`)
...
// 					break
			...
			case 'som':
				...
				//if (!isUser) return reply(mess.only.userB)
				//if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return frhan.sendMessage(from, 'Não sei o que tava escrito aqui', text, {quoted: mek})
				const gtts = require('./lib/gtts')(args[0])
				if (args.length < 2) return frhan.sendMessage(from, 'Cade o texto animal?', text, {quoted: mek})
				dtt = body.slice(8)
				ranm = getRandom('.mp3')
				rano = getRandom('.ogg')
				dtt.length > 150
				? reply('Escreve um texto menor porra, vai tomar no seu cu!')
...
// 					break 
			    ...
			    //if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 Boa noite 〙', members_id, true)
					break
			    // case 'kbbi':
			    // try {
			    // if (isBanned) return reply(mess.only.benned)    
			    ...
			    //if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tu é burro?')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
			    ...
			    //if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Escolha o Admin')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '? :\n'
						for (let _ of mentioned) {
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Use ON para ligar e OFF para desligar.')
					if ((args[0]) === 'on') {
						...
						if (!isAntiLink) return reply('NÃO VOU TRADUZIR')
						_antilink.splice(from, 1)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(_antilink))
						reply(`✓`)
					} else {
						reply('Use ON para ligar e OFF para desligar.')
					}
					break 
// 				case 'badword':
// 				if (!isGroup) return reply(mess.only.group)
